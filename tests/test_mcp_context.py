...
from local_code_rag import mcp_context, mcp_server  # noqa: E402
...
class MCPContextTests(unittest.TestCase):
    def test_repository_metadata_discovery(self) -> None:
        ...
        with patch("local_code_rag.mcp_context._open_collection", return_value=fake):
            self.assertEqual(
    ...
    def test_get_repository_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config = _config("/tmp/db")
            with patch(
                "local_code_rag.mcp_context._open_collection", return_value=fake
            ):
    ...
    def test_search_code_with_and_without_repo(self) -> None:
        ...
        with patch(
            "local_code_rag.mcp_context.search_chunks", side_effect=fake_search_chunks
        ):
            with patch(
                "local_code_rag.query.ollama_chat",
    ...
    def test_no_ollama_chat_during_retrieval_tools(self) -> None:
        with patch(
            ...
            with patch(
                ...
                with patch("local_code_rag.mcp_context.search_chunks", return_value=[]):
